...
###++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++###
    ...
    public function generate_token($length = 8) {
        ...
        for ($i = 0, $token = ''; $i < $length; $i++) {
            ...
            $token .= substr($characters, $index, 1);
        }
        return $token;
    }
...
// User exists, now validate credentials.
        if ($query->num_rows() == 1) {
            ...
            $query_profile = $this->db->select('upro_type')
                    ->where('upro_uacc_fk', $user->uacc_id)
                    ->get('user_profiles');
            $result_profile = $query_profile->row();

// Checking if user is customer olny.
//if ($result_profile->upro_type != 'customer') {
//return FALSE;
// }
...
// Checking if account doent belongs to agent of an organization.
            if ($user->uacc_group_fk != 3) {
                $this->set_error_message('Other than surveyors no one is allowed to login to this portal , Please visit <a href="socialsurvey.com">socialsurvey.com</a> if your not a surveyor.', 'Public', FALSE);
                return FALSE;
            }
